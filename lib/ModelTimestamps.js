...
module.exports = function(schema, options){
	...
	/**
	 * Before find hook which checks if there is a $scope used in the search
	 * If used then search documents that are not deleted
	 * @param  {[type]} next){		if(typeof this.options.$scope ! [description]
	 * @return {[type]}                    [description]
	 */
	schema.pre("find", function(next){
		if(typeof this.options.$scope !== "undefined"){
			this.where({delete_time: {
				$ne: null
			}});
		}
